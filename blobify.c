...
{
    if (blob && blb_range_in(blob->range, value))
    {
...
bool blb_blob_block_put(blb_blob_t *blob, int32_t offset, uint8_t value){
    if(!blob || !blob->block) return false;
    return blb_block_put(blob->block, offset, value);
}

bool blb_blob_block_get(blb_blob_t *blob, int32_t offset, uint8_t *value){
    if(!blob || !blob->block) return false;
    return blb_block_get(blob->block, offset, value);
}
